-- Remplace la RPC init_system : évite ON CONFLICT sur des colonnes sans contrainte UNIQUE (erreur 42P10).
-- Idempotent : CREATE OR REPLACE. Premier déploiement : crée public.paroisses si absent.
-- Supprime d’abord toute surcharge (ex. json vs jsonb) : sinon CREATE OR REPLACE ne remplace pas l’autre version.
...
BEGIN
  ...
  IF v_hdr_id IS NOT NULL THEN
    ...
    WHERE id = v_hdr_id;
  ELSE
    -- Pas de created_at/updated_at explicites : certaines BDD n’ont pas created_at sur header_config (schéma ancien).
